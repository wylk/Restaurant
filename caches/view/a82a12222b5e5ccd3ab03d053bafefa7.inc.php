...
<a class="button collect <?php if(!$favorite)echo 'collect-btn';?>  fl" data-skuid="<?php echo $goods['sku_id'];?>" data-name="<?php echo $goods['sku_name'];?>" data-url="<?php echo urlencode($_SERVER['REQUEST_URI']);?>" data-price="<?php echo $goods['shop_price'];?>"><?php if($favorite){echo "已收藏";}else if(!$favorite){echo "收藏商品";};?></a>
...
<?php foreach($specs['value'] as $k => $spec):?>
...
<div class="square-check pic-center" title="<?php echo $spec;?>"><img title="<?php echo $spec;?>" src="<?php echo $specs['img'][$k];?>"></div>
<?php } else { ?>
...
<?php echo runhook('detail_goods_info');?>
</div>
...
<?php
	...
	$data = $taglib_promotion_prom_group->lists(array('sku_id'=>$_GET[sku_id]), array('limit'=>'20','cache'=>'de4532f22f87971b6f545d4b3d918a04,3600'));
?>
<div class="container fitting-suit border border-gray-white">
<ul class="p-tab-first clearfix">
...
<ul class="tab p-tab-trigger-wrap p-tab-second clearfix margin-big-bottom"><?php if(is_array($data)) foreach($data as $k => $group){?><li class="ui-switchtab-item tab-second-lists" data-id="<?php echo $group['id'];?>"><a href="javascript:;"><?php echo $k;?></a></li>
...
<span class="padding-bottom border-bottom border-dotted">商品总价：<em class="text-big text-mix">￥<em class="price"><?php echo $goods['prom_price']?></em></em></span>
...
<div id="pro-detail-right" class="fr right">
...
<?php
	...
	$data = $taglib_order_order->records(array('sid'=>$goods[spu_id],'isspu'=>'true'), array('limit'=>'8','cache'=>'d069000964675468e676d80335b0d14d,3600','page'=>$_GET[page],'pagefunc'=>'pages'));
...
<?php } else { ?>
<div class="fr margin-right text-lh-large">商品已售罄</div>
<?php } ?>
</li>
<?php if($qr_code[0]>0){?>
...
<div id="pro-all-comment" class="margin-big-top border border-gray-white product-detail-panel item-blue-top pro-border-top-gray">
...
<div id="pro-goods-consult" class="margin-big-top border border-gray-white product-detail-panel item-blue-top">
<ul class="tab p-tab-trigger-wrap">
...
<div id="pro-all-consult" class="margin-big-top border border-gray-white product-detail-panel item-blue-top pro-border-top-gray">
<ul class="tab p-tab-trigger-wrap">
...
<?php
	...
	$data = $taglib_goods_consult->lists(array('spu_id'=>$goods[spu_id]), array('limit'=>'5','cache'=>'bf381dc9c60ab91179490ce7cf87e80b,3600'));
?>
<?php if(empty($data)){?>
...
<?php
	$taglib_goods_consult = new taglib('goods','consult');
	$data = $taglib_goods_consult->count(array('spu_id'=>$goods[spu_id]), array('limit'=>'20','cache'=>'a1af28e23fd69761885c5a516d9a62a9,3600'));
?>
<?php if($data>5){?>
...
<?php if($cache[invoice_vat_enabled] == 1){?>
<div id="pro-buy-record" class="margin-big-top border border-gray-white product-detail-panel item-blue-top">
<ul class="tab p-tab-trigger-wrap">
<li class="ui-switchtab-item"><a class="text-sub" href="javascript:;" id="record_btn">购买记录(0)</a></li>
</ul>
<div class="ui-switch-panel ui-selected padding">
...
$.get("<?php echo url('ajax_get_favorite')?>",{sku_id:'<?php echo $_GET["sku_id"]?>'},function(ret){
if(ret.status == 1){
$(".collect").removeClass('collect-btn');
$(".collect").html('<em class="text-gray">已收藏</em>');
}else{
